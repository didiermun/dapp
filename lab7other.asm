...
     ; set FGCR = 0 to stop playing note 
 ...
 ;;which indicates that a key has been pressed 
...
     ; retrieving the ascii value of the key pressed from the KPDR and storing it into R0

 BRz WAIT_FOR_KEY ; Branch If zero, no key yet. Loop back 
...
ADD R0, R0, -'1'   ; Convert ASCII to numeric value

BRn WAIT_FOR_KEY   ; If result is negative, loop back

 ;; now R0 contains the index representing pressed key 
 ...
 BRp WAIT_FOR_KEY ; if positive, branch back to wait for the key
...
LEA R2, NOTE_FREQ_ARR ; Load note frequency address array into R2

ADD R2, R2, R0 ; Add the index to get the address of the specific note frequency

LDR R1, R2, #0 ; Load frequency value into R1 from the array

STI R1, FGDR_ADDR ; Store frequency in FGDR address to set the note frequency

AND R1, R1, #0 ; Clear R1

ADD R1, R1, #1 ; Set R1 to a non-zero value

STI R1, FGCR_ADDR ; Set a non-zero value to FGCR to start playing the note
...
WAIT_FOR_RELEASE 
 ...
 LDI R1, KPSR_ADDR ; Load value of KPSR into R1

 BRzp WAIT_FOR_RELEASE ; If KPSR is not zero or positive, keep waiting for release
...
KEY_RELEASED  
 ...
 STI R0, FGCR_ADDR ; Store zero to FGCR to stop note
